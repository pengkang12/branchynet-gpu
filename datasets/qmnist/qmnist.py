...
def get_qmnist(filedir=""):
    # the qmnist training set, download from the web if not found
    qtrain = QMNIST(filedir+'_qmnist', train=True, download=True)

    # the qmnist testing set, do not download.
    qtest = QMNIST(filedir+'_qmnist', train=False, download=True)

    Q_X_train = qtrain.data.numpy().astype(np.float32) / 255
    Q_X_train = Q_X_train.reshape(Q_X_train.shape[0], 1, 28, 28)
    Q_Y_train = qtrain.targets.numpy()[:,0:1]
 

    Q_X_test = qtest.data.numpy().astype(np.float32) / 255
    Q_X_test = Q_X_test.reshape(Q_X_test.shape[0], 1, 28, 28)
    Q_Y_test = qtest.targets.numpy()[:,0:1]
 
    return Q_X_train, Q_Y_train, Q_X_test, Q_Y_test
